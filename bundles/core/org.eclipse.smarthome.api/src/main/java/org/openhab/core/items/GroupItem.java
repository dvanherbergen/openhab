...
public class GroupItem extends GenericItem implements StateChangeListener {
	...
	public List<Class<? extends State>> getAcceptedDataTypes() {
		if(baseItem!=null) {
			...
			List<Class<? extends State>> acceptedDataTypes = new ArrayList<Class<? extends State>>();
			
			for(Item item : members) {
				if(acceptedDataTypes.isEmpty()) {
					acceptedDataTypes = item.getAcceptedDataTypes();
				} else {
					for (Class<? extends State> s : acceptedDataTypes) {
						if (!item.getAcceptedDataTypes().contains(s)) {
							acceptedDataTypes.remove(s);
						}
					}
				}
			}
	...
	public List<Class<? extends Command>> getAcceptedCommandTypes() {
		if(baseItem!=null) {
			...
			List<Class<? extends Command>> acceptedCommandTypes = new ArrayList<Class<? extends Command>>();
			
			for(Item item : members) {
				if(acceptedCommandTypes.isEmpty()) {
					acceptedCommandTypes = item.getAcceptedCommandTypes();
				} else {
					for (Class<? extends Command> s : acceptedCommandTypes) {
						if (!item.getAcceptedCommandTypes().contains(s)) {
							acceptedCommandTypes.remove(s);
						}
					}
				}
			}
	...
	protected void internalSend(Command command) {
		if(eventPublisher!=null) {
			for(Item member : members) {
				...
				eventPublisher.postCommand(member.getName(), command);
			}		
		}
	}
