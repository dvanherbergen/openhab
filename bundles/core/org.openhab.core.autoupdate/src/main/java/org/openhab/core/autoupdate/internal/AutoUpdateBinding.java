 ...
 * </p>
 * <p>
 * For example when implementing validation steps before changing a State one
 * needs to control the State update oneself.
 * </p>
 * 
...
public class AutoUpdateBinding implements Binding {
	 ...
	 * @param command
	 *            the command being received and posted as {@link State} update
	 *            if <code>command</code> is instance of {@link State} as well.
	 */
	@Override
				...
				if (item.getAcceptedDataTypes().contains(newStatus.getClass())) {
					...
					for (Class<? extends State> state : item.getAcceptedDataTypes()) {
						try {
							...
							logger.warn("InstantiationException on ", e.getMessage()); // Should
																						// never
																						// happen
						} catch (IllegalAccessException e) {
							logger.warn("IllegalAccessException on ", e.getMessage()); // Should
																						// never
																						// happen
						}
					}
